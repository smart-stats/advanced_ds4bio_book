...
\end{align*}
$$
where, recall $E[Y ~|~ Z, O(Z)] = O(Z)$ and $E[O(Z)~|~Z] = O(Z)$ since $O(Z)$ is coarser than $Z$.
...
$$
Consider a classifier that is finer. Then, $E[Y ~|~ Z, \phi(Z)] = E[Y  ~|~ Z] = O(Z)$ since $Z$ is finer than $O(Z)$, we have that the conditional independence holds is equivalent to
$$
O(Z) = E[Y ~|~ \phi(Z)].
$$
...
For 4. $\phi$ is calibrated iff $\phi(Z) = E[Y~|~\phi(Z)]$. But
$$
...
valdat = valdat.merge(grid,how = "outer").fillna(0)
```
